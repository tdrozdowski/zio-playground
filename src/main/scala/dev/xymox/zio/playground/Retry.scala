...
object Main extends App {
  ...
  override def run(args: List[String]): URIO[ZEnv, ExitCode] =
    for {
      ...
      (result, count) = r
      _              <- zio.console.putStrLn(s"result is: $result, final count: $count").orDie
      result         <- ZIO.fromEither(result).mapError(v => new IllegalArgumentException(v)).orDie
      _              <- console.putStrLn(s"The successful result is: $result").orDie
    } yield ExitCode.success

  val doOrFail: ZIO[Random with Console, String, String] =
    putStrLn("trying....").orDie *> zio.random.nextDouble.flatMap(d => if (d < 0.5) ZIO.fail("failure!") else ZIO.succeed("success!"))
